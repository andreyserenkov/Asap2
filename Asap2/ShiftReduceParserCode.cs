 ...
 where TSpan : IMerge<TSpan>, new()
    {
        ...
        private IDictionary<string, Token> reidentification =  new Dictionary<string, Token>();
        ...
        private string GetPrevTokenName()
        {
            ...
            if (valStack is Asap2.ValueType vsValue)
                return vsValue.s;
            return string.Empty;
        }
        public bool Parse()
            ...
            while (true)
            {
                ...
                if (FsaState.ParserTable != null)
                {
                    if (NextToken == 0)
                    {
                        ...
                        if ((reidentification.ContainsKey(TerminalToString(NextToken))) && GetPrevTokenName() != "/begin" && GetPrevTokenName() != "/end")
                        {
                            var to_token = reidentification[TerminalToString(NextToken)];
#if TRACE_ACTIONS
                            Console.Error.WriteLine("Reidentification token: {0} to {1}", TerminalToString(NextToken), to_token);
#endif
                            NextToken = (int)to_token;
                        }
#if TRACE_ACTIONS
